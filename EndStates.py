...
def returnEndStates(model, phrases):
  endstates = {}
  for i in phrases:
    formatted = np.array(encodeDecode(i, comp1.indexToDur)).reshape(-1, 1)
    states = model.predict(formatted)
    lastState = int(states[-1])
    endstates[lastState] = endstates.get(lastState, 0) + 1 
  return endstates
...
shorter = [i for i in comp1.encodedPhrases if len(i) >= 4 and len(i) <= 10]
medium = [i for i in comp1.encodedPhrases if len(i) >= 10 and len(i) <= 15]
long = [i for i in comp1.encodedPhrases if len(i) >= 15 and len(i) <=25]

s = returnEndStates(model, shorter)
m = returnEndStates(model, medium)
l = returnEndStates(model, long)
